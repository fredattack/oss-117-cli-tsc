...
export const displayQuotes = (apiResponse: Quote[] | Quote, number:number|null) => {
    if(Array.isArray(apiResponse) && apiResponse.length > 0){
        if(number && number > apiResponse.length){
            console.log(consoleColors.red,`${consoleIcones.warning} Il n'y a que ${apiResponse.length} citations disponibles.`)
...
export const displayCharacters = (apiResponse: Character[] | Character) => {
    if(Array.isArray(apiResponse) && apiResponse.length > 0){
        apiResponse.forEach((character) => {
            ...
            console.log(consoleColors.blue, `"${character.name}" - alias ${character.slug}`);
        });
    }
    else if(typeof apiResponse === 'object' && 'name' in apiResponse){
        console.log(consoleColors.blue, `"${apiResponse.name}" - alias ${apiResponse.slug}`);
    }
};

export const displayUnknownCharacters = (characters:  Character[]) => {
    console.log(consoleColors.red,`${consoleIcones.warning} Ce personnage n'existe pas.\n`);
